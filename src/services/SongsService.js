...
class SongService {
  ...
  async getSong(title, performer) {
    const query = 'SELECT song_id as id, title, performer FROM songs';
    const result = await this.#pool.query(query);
  ...
  async putSongById(id, title, year, genre, performer, duration, albumId) {
    const query = {
      text: 'UPDATE songs SET title = $1, year = $2, performer = $3, genre = $4, duration = COALESCE($5, duration), album_id = COALESCE($6, album_id) WHERE song_id = $7 RETURNING song_id',
