...
class AlbumService {
  ...
  async postAlbum({ name, year }) {
    ...
    const query = {
      text: 'INSERT INTO album VALUES ($1, $2, $3) RETURNING album_id as "albumId"',
      values: [id, name, year],
    };
  ...
  async getAlbumById(id) {
    const queryAlbum = {
      text: 'SELECT album_id as id, name, year FROM album WHERE album_id = $1',
      values: [id],
    };

    const querySong = {
      text: 'SELECT song_id as id, title, year, performer, genre, duration, album_id as "albumId" FROM song WHERE album_id = $1',
      values: [id],
    };
